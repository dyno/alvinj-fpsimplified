...
object ScalazLensExample extends App {
  ...
  val userRatingLens2 =
    userGeneralInfoLens.andThen(generalInfoSiteInfoLens).andThen(siteInfoRatingLens)

  //compose
  val userRatingLens3 = siteInfoRatingLens <=< generalInfoSiteInfoLens <=< userGeneralInfoLens
  val userRatingLens4 =
    siteInfoRatingLens.compose(generalInfoSiteInfoLens).compose(userGeneralInfoLens)
  ...
  val isConfirmedLens = (userBillInfoLens >=> billInfoAddressesLens) =>= {
    _.map(_.copy(isConfirmed = true))
  }
